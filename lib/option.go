...
var OptionMap = map[string]Option{
	OptionConfigFile: {
		"-c", "--config-file", "", OptionTypeString, "", "",
	...
	OptionAccessKeyID:     {"-i", "--access-key-id", "", OptionTypeString, "", "", "访问oss使用的AccessKeyID（该选项值会覆盖配置文件中的相应设置）。", "AccessKeyID while access oss(Notice that the value of the option will cover the value in config file)."},
	OptionAccessKeySecret: {"-k", "--access-key-secret", "", OptionTypeString, "", "", "访问oss使用的AccessKeySecret（该选项值会覆盖配置文件中的相应设置）。", "AccessKeySecret while access oss(Notice that the value of the option will cover the value in config file)."},
	OptionSTSToken:        {"-t", "--sts-token", "", OptionTypeString, "", "", "访问oss使用的STSToken（该选项值会覆盖配置文件中的相应设置），非必须设置项。", "STSToken while access oss(Notice that the value of the option will cover the value in config file), not necessary."},
	OptionLimitedNum:      {"", "--limited-num", strconv.Itoa(DefaultLimitedNum), OptionTypeInt64, strconv.FormatInt(MinLimitedNum, 10), "", "返回结果的最大个数。", "the limited number of return results."},
	OptionMarker:          {"", "--marker", "", OptionTypeString, "", "", "列举Buckets时的marker，或列举objects或Multipart Uploads时的key marker, 或者其他有需要marker的地方。", "the marker of bucket when list buckets, or the marker of key when list object or Multipart Uploads, Or other places where a marker is needed"},
	OptionUploadIDMarker:  {"", "--upload-id-marker", "", OptionTypeString, "", "", "列举Multipart Uploads时的uploadID marker。", "the marker of object when list object or Multipart Uploads."},
	OptionACL:             {"", "--acl", "", OptionTypeString, "", "", "acl信息的配置。", "acl information."},
	OptionShortFormat:     {"-s", "--short-format", "", OptionTypeFlagTrue, "", "", "显示精简格式，如果未指定该选项，默认显示长格式。", "Show by short format, if the option is not specified, show long format by default."},
	OptionDirectory:       {"-d", "--directory", "", OptionTypeFlagTrue, "", "", "返回当前目录下的文件和子目录，而非递归显示所有子目录下的所有object。", "Return matching subdirectory names instead of contents of the subdirectory."},
	OptionMultipart:       {"-m", "--multipart", "", OptionTypeFlagTrue, "", "", "指定操作的对象为bucket中未完成的Multipart事件，而非默认情况下的object。", "Indicate that the subject of the command are uncompleted Multipart Uploads, instead of objects(which is the subject in default situation."},
	OptionAllType:         {"-a", "--all-type", "", OptionTypeFlagTrue, "", "", "指定操作的对象为bucket中的object和未完成的Multipart事件。", "Indicate that the subject of the command contains both objects and uncompleted Multipart Uploads."},
	OptionRecursion:       {"-r", "--recursive", "", OptionTypeFlagTrue, "", "", "递归进行操作。对于支持该选项的命令，当指定该选项时，命令会对bucket下所有符合条件的objects进行操作，否则只对url中指定的单个object进行操作。", "operate recursively, for those commands which support the option, when use them, if the option is specified, the command will operate on all match objects under the bucket, else we will search the specified object and operate on the single object."},
	OptionBucket:          {"-b", "--bucket", "", OptionTypeFlagTrue, "", "", "对bucket进行操作，该选项用于确认操作作用于bucket", "the option used to make sure the operation will operate on bucket"},
	OptionStorageClass: {
		"", "--storage-class", DefaultStorageClass, OptionTypeAlternative, fmt.Sprintf("%s/%s/%s/%s", StorageStandard, StorageIA, StorageArchive, StorageColdArchive), "",
	...
	OptionForce:  {"-f", "--force", "", OptionTypeFlagTrue, "", "", "强制操作，不进行询问提示。", "operate silently without asking user to confirm the operation."},
	OptionUpdate: {"-u", "--update", "", OptionTypeFlagTrue, "", "", "更新操作", "update"},
	OptionDelete: {"", "--delete", "", OptionTypeFlagTrue, "", "", "删除操作", "delete"},
	OptionOutputDir: {
		"", "--output-dir", DefaultOutputDir, OptionTypeString, "", "",
	...
	OptionBigFileThreshold: {
		"", "--bigfile-threshold", strconv.FormatInt(DefaultBigFileThreshold, 10), OptionTypeInt64, strconv.FormatInt(MinBigFileThreshold, 10), strconv.FormatInt(MaxBigFileThreshold, 10),
	...
	OptionPartSize: {
		"", "--part-size", strconv.FormatInt(DefaultPartSize, 10), OptionTypeInt64, strconv.FormatInt(MinPartSize, 10), strconv.FormatInt(MaxPartSize, 10),
	...
	OptionDisableCRC64: {"", "--disable-crc64", "", OptionTypeFlagTrue, "", "", "该选项关闭crc64，默认情况下，ossutil进行数据传输都打开crc64校验。", "Disable crc64, in default situation, ossutil open crc64 check when transmit data."},
	OptionCheckpointDir: {
		"", "--checkpoint-dir", CheckpointDir, OptionTypeString, "", "",
	...
	OptionSnapshotPath: {
		"", "--snapshot-path", "", OptionTypeString, "", "",
	...
	OptionRetryTimes: {
		"", "--retry-times", strconv.Itoa(RetryTimes), OptionTypeInt64, strconv.FormatInt(MinRetryTimes, 10), strconv.FormatInt(MaxRetryTimes, 10),
	...
	OptionRoutines: {
		"-j", "--jobs", strconv.Itoa(Routines), OptionTypeInt64, strconv.FormatInt(MinRoutines, 10), strconv.FormatInt(MaxRoutines, 10),
	...
	OptionParallel: {
		"", "--parallel", "", OptionTypeInt64, strconv.FormatInt(MinParallel, 10), strconv.FormatInt(MaxParallel, 10),
	...
	OptionRange: {"", "--range", "", OptionTypeString, "", "", "下载文件时，指定文件下载的范围，格式为：3-9或3-或-9", "the range when download objects, the form is like: 3-9 or 3- or -9"},
	OptionEncodingType: {
		"", "--encoding-type", "", OptionTypeAlternative, URLEncodingType, "",
	...
	OptionTimeout: {
		"", "--timeout", strconv.FormatInt(DefaultTimeout, 10), OptionTypeInt64, strconv.FormatInt(MinTimeout, 10), strconv.FormatInt(MaxTimeout, 10),
	...
	OptionLanguage: {
		"-L", "--language", DefaultLanguage, OptionTypeAlternative, fmt.Sprintf("%s/%s", ChineseLanguage, EnglishLanguage), "",
	...
	OptionHashType: {
		"", "--type", DefaultHashType, OptionTypeAlternative, fmt.Sprintf("%s/%s", DefaultHashType, MD5HashType), "", fmt.Sprintf("计算的类型, 默认值：%s, 取值范围: %s/%s", DefaultHashType, DefaultHashType, MD5HashType),
		fmt.Sprintf("hash type, Default: %s, value range is: %s/%s", DefaultHashType, DefaultHashType, MD5HashType),
	},
	OptionVersion:      {"-v", "--version", "", OptionTypeFlagTrue, "", "", fmt.Sprintf("显示ossutil的版本（%s）并退出。", Version), fmt.Sprintf("Show ossutil version (%s) and exit.", Version)},
	OptionRequestPayer: {"", "--payer", "", OptionTypeString, "", "", "请求的支付方式，如果为请求者付费模式，可以将该值设置成\"requester\"", "The payer of the request. You can set this value to \"requester\" if you want pay for requester"},
	OptionLogLevel: {
		"", "--loglevel", "", OptionTypeString, "", "",
	...
	OptionDisableEmptyReferer: {
		"", "--disable-empty-referer", "", OptionTypeFlagTrue, "", "",
	...
	OptionOnlyCurrentDir: {
		"", "--only-current-dir", "", OptionTypeFlagTrue, "", "",
	...
	OptionDisableEncodeSlash: {
		"", "--disable-encode-slash", "", OptionTypeFlagTrue, "", "",
	...
	OptionDisableDirObject: {
		"", "--disable-dir-object", "", OptionTypeFlagTrue, "", "",
	...
	OptionRedundancyType: {
		"", "--redundancy-type", "", OptionTypeString, "", "",
	...
	OptionDisableAllSymlink: {
		"", "--disable-all-symlink", "", OptionTypeFlagTrue, "", "",
	...
	OptionSkipVerifyCert: {
		"", "--skip-verify-cert", "", OptionTypeFlagTrue, "", "",
		"表示不校验服务端的数字证书",
	...
	OptionCloudBoxID: {
		"", "--cloudbox-id", "", OptionTypeString, "", "",
		"云盒的id，缺省值为空，适用于云盒场景",
		"The ID of the cloud box. The default value is empty. It is applicable to cloud box scenarios",
	},
	OptionQueryParam: {
		"", "--query-param", "", OptionTypeStrings, "", "",
		"设置请求的query参数",
		"Set the query parameters for the request",
	},
	OptionForcePathStyle: {
		"", "--force-path-style", "", OptionTypeFlagTrue, "", "",
		"使用 path style 访问方式",
		"Use path-style access ",
	},
	OptionRuntime: {
		"", "--runtime", "", OptionTypeInt64, "", "",
		"设置命令的持续的运行时间",
		"specifies the max running time of the command.",
	},
	OptionIgnoreHiddenFile: {"", "--ignore-hidden-file", "", OptionTypeFlagTrue, "", "", "忽略隐藏文件", "ignore hidden file."},
}
